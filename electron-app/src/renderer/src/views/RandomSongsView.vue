  ...
  <div class="flex flex-col gap-2" style="margin-left: 30px;">
    ...
    <div class="flex gap-4 justify-self-start">
      ...
      <div class="flex-1 flex flex-col justify-between gap-2 h-[210px]">
        <div class="text-xl font-bold">{{ playlistTitle }}</div>
        <div class="text-xs text-on-secondary">{{ playlistDesc }}</div>
...
const playlistTitle = ref('轻音乐')
const playlistDesc = ref('放松心情的音乐')
const collectCount = ref(15)

const formattedPlayCount = computed(() => `${songs.value.length}首`)
...
async function fetchRandomSongs() {
  try {
    ...
    const allSongs = res.songs || res.data?.songs || []
    songs.value = getRandomItems(allSongs, 10).map(song => ({
      ...song,
