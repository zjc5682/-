    ...
    <div class="flex gap-4 justify-self-start">
      ...
      <div class="flex-1 flex flex-col justify-between gap-2 h-[210px]">
        ...
        <div class="flex items-center gap-3 text-xs">
          ...
          <span>{{ playlistInfo.creator || '官方' }}</span>
...
function handleSongClick(song, index) {
  console.log('准备播放歌曲:', song.title, 'URL:', song.url)
...
async function fetchPlaylistInfo() {
  ...
  if (!id) return
  try {
    const res = await request.get(`/playlists/${id}`)
    const info = res.playlist || res.data?.playlist || {}
    if (info.title) {
      playlistInfo.value = {
        ...playlistInfo.value,
        id: info.id,
...
async function fetchAll() {
  await Promise.all([fetchPlaylistInfo(), fetchSongs()])
}
